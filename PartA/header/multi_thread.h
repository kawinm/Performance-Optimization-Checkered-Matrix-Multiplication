...
int Ng, *matAg, *matBg, *outputg;

// Create other necessary functions here
void *multiply_row(void *data) {

	int colSetB = * (int *) data;
	
	int N = Ng, *matA = matAg, *matB = matBg, *output = outputg;
...
{    
    ...
    for(int colSetB = 0; colSetB < N; colSetB += 48) {
    	...
    	for (int i = 0; i < NUM_THREADS; i++ ) {
	        ...
	        rc = pthread_create(&threads[i], NULL, multiply_row, &td[i]);
